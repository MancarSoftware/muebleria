...
import { ArrowDownRight, ArrowRight, Check, Clock3, Mail, MapPin, Menu, Phone, Sparkles, X } from 'lucide-react';
...
const steps = [
  ['Cuéntanos tu espacio', 'Tus ideas, medidas o incluso una foto. Empezamos desde donde estés.'],
  ['Probamos posibilidades', 'Vemos materiales y proporciones para que puedas decidir con claridad.'],
  ['Eliges con certeza', 'Visita el showroom y conoce las piezas antes de llevarlas a casa.'],
];
const pieces = [
    ...
    <main>
      <section className="sales-hero" id="inicio">
        <div className="sales-hero-copy">
          ...
          <motion.div className="sales-hero-actions" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .22 }}><a className="sales-button" href="#contacto">Cuéntanos tu espacio <ArrowRight/></a><a className="sales-link" href="#piezas">Ver piezas <ArrowDownRight/></a></motion.div>
      ...
      <section className="sales-intro"><div className="sales-intro-card"><img src={bedImage} alt="Dormitorio sereno amoblado por Casa Nativa"/><div><span>CASA NATIVA</span><p>Elegir muebles no debería ser complicado.</p></div></div><div className="sales-intro-copy"><p className="sales-eyebrow">UNA MEJOR FORMA DE ELEGIR</p><h2>No necesitas saber de diseño.<br/>Necesitas sentir que <em>todo encaja.</em></h2><p>Te acompañamos a encontrar piezas que respondan a tu espacio, tu rutina y tu forma de habitar; no a una tendencia pasajera.</p></div></section>

      <section className="sales-process" id="servicios"><div className="sales-section-heading"><p className="sales-eyebrow">ASÍ EMPEZAMOS</p><h2>De una idea a un espacio<br/>que se <em>siente tuyo.</em></h2></div><div className="sales-step-grid">{steps.map(([title, description], index) => <motion.article key={title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .2 }} transition={{ duration: .45, delay: index * .08 }}><h3>{title}</h3><p>{description}</p></motion.article>)}</div></section>

      <section className="sales-pieces" id="piezas"><div className="sales-section-heading"><p className="sales-eyebrow">PIEZAS PARA LA VIDA REAL</p><h2>Piezas que le dan<br/><em>sentido a tu espacio.</em></h2></div><div className="sales-piece-grid">{pieces.map((piece, index) => <motion.figure key={piece.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .15 }} transition={{ duration: .45, delay: index * .06 }}><div><img src={piece.image} alt={piece.title}/></div><figcaption><small>{piece.category}</small><strong>{piece.title}</strong></figcaption></motion.figure>)}</div></section>
