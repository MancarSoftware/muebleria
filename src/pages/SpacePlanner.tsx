  ...
  return <section className="space-planner">
    ...
    <div className="space-layout">
      ...
      <aside className="space-brief">
        ...
        {!selectedItems.length ? <div className="space-general-inquiry"><p className="eyebrow">¿AÚN NO TIENES PIEZAS?</p><p>Cuéntanos qué necesitas y te ayudamos a empezar sin elegir un producto todavía.</p><Link to="/contact">Enviar una consulta <ArrowRight/></Link></div> : saveState === 'saved' ? <div className="proposal-success"><Check/><div><b>Tu propuesta quedó registrada.</b><span>Ahora puedes enviarla al showroom por WhatsApp.</span></div></div> : <button className="dark-button full" type="button" onClick={saveProposal} disabled={saveState === 'saving'}>{saveState === 'saving' ? 'Guardando propuesta…' : 'Guardar y continuar'} <ArrowRight/></button>}
