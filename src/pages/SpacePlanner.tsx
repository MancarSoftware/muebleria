...
import { spaceRoomTypes, type SpaceRoom, useSpacePlanner } from '../hooks/useSpacePlanner';
...
export function SpacePlanner() {
  ...
  const [isAddingReviewPiece, setIsAddingReviewPiece] = useState(false);
  ...
  const reviewItems = useMemo(() => selectedItems.filter((item) => reviewProductIds.includes(item.product.id)), [reviewProductIds, selectedItems]);
  const reviewCandidates = selectedItems.filter((item) => !reviewProductIds.includes(item.product.id));
  ...
  const proposalItems = proposalRoute === 'review' ? reviewItems : selectedItems;
  const proposalTotal = proposalItems.reduce((sum, item) => sum + item.product.price, 0);
  ...
  const chooseProposalRoute = (route: Exclude<ProposalRoute, 'choose'>) => {
    ...
    if (route === 'review') { setReviewProductIds([]); setIsAddingReviewPiece(false); }
    setSaveState('idle');
    setSaveError('');
  };

  const addToReview = (productId: string) => {
    setReviewProductIds((current) => current.includes(productId) ? current : [...current, productId]);
    setReviewRoomIndex(Math.max(activeGroups.length - 1, 0));
    setIsAddingReviewPiece(false);
  };

  const removeFromReview = (productId: string) => {
    setReviewProductIds((current) => current.filter((id) => id !== productId));
    setReviewRoomIndex(0);
  };

  const saveProposal = async () => {
    if (!proposalItems.length || needsContact) {
      setSaveState('error');
    ...
    try {
      ...
      trackEvent('generate_lead', { currency: 'USD', value: proposalTotal, room_type: proposalRoom, item_count: proposalItems.length, room_count: activeGroups.length });
      setSaveState('saved');
    } catch (error) {
      setSaveState('error');
    ...
    <div className="space-layout">
      <section className="space-selection">
        ...
        {selectedItems.length ? <div className="space-items">{selectedItems.map(({ product, colorName, roomId }) => {
          ...
          const requirement = group?.estimate.requirements.find((item) => item.productId === product.id) ?? estimateSpace([product]).requirements[0];
          return <article key={product.id} className="space-item"><img src={image} alt={product.name} loading="lazy" decoding="async"/><div><p className="eyebrow">{product.category}</p><h3>{product.name}</h3><b>${product.price.toLocaleString('en-US')}</b><div className="space-item-options"><label>Color<select value={colorName ?? selectedColor?.name ?? ''} onChange={(event) => planner.updateColor(product.id, event.target.value)}>{variants.map((variant) => <option key={variant.id}>{variant.name}</option>)}</select></label></div>{requirement && <small className="space-item-rule">Huella: {(requirement.widthCm / 100).toFixed(2)} × {(requirement.depthCm / 100).toFixed(2)} m · paso recomendado: {requirement.clearanceCm} cm</small>}</div><button className="space-remove" type="button" onClick={() => planner.remove(product.id)} aria-label={`Quitar ${product.name}`}><Trash2/></button></article>;
      ...
      <aside className="space-brief">
        {!selectedItems.length ? <div className="space-general-inquiry"><p className="eyebrow">¿AÚN NO TIENES PIEZAS?</p><h2>Empecemos por<br/><em>lo que buscas.</em></h2><p>Cuéntanos qué necesitas y te ayudamos a empezar sin elegir un producto todavía.</p><Link to="/contact#contact-form">Enviar una consulta <ArrowRight/></Link></div> : proposalRoute === 'choose' ? <div className="proposal-paths"><p className="eyebrow">¿CÓMO QUIERES CONTINUAR?</p><h2>Tu selección<br/>ya está <em>lista.</em></h2><p>Puedes solicitar todas las piezas directamente o revisar solo las que te generen dudas de medidas.</p><button className="proposal-path" type="button" onClick={() => chooseProposalRoute('direct')}><span><Check/></span><div><b>Solicitar estas piezas</b><small>Confirma disponibilidad, acabados y entrega.</small></div><ArrowRight/></button><button className="proposal-path" type="button" onClick={() => chooseProposalRoute('review')}><span><Ruler/></span><div><b>¿Tienes dudas sobre las medidas?</b><small>Elige solo las piezas que quieres comprobar y dónde las ubicarías.</small></div><ArrowRight/></button><Link className="proposal-add-items" to="/catalog">Agregar otra pieza <ArrowRight/></Link></div> : <>
          ...
          {saveState === 'saved' && <a className="dark-button full proposal-whatsapp" href={whatsappLink(message)} onClick={() => trackEvent('contact_whatsapp', { location: proposalRoute === 'review' ? 'space_review' : 'space_request', item_count: proposalItems.length, value: proposalTotal, currency: 'USD' })} target="_blank" rel="noreferrer">{proposalRoute === 'review' ? 'Abrir WhatsApp con mi revisión' : 'Abrir WhatsApp para consultar'} <ArrowRight/></a>}
          ...
          </> : <div className="review-awaiting-piece"><Ruler/><p><b>Primero elige una pieza para revisar.</b> Solo después te pediremos las medidas del ambiente donde planeas ubicarla.</p><button type="button" onClick={() => chooseProposalRoute('direct')}>Solicitar toda mi selección sin revisar medidas <ArrowRight/></button></div>}
        </>}
      </aside>
    </div>
  </section>;
