...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card">
    <Link to={`/catalog/${product.slug}`}><img src={product.images[0]} alt={product.name} loading="lazy"/><span>{product.category}</span><h3>{product.name}</h3><b>${product.price.toLocaleString('en-US')}</b><ArrowUpRight className="corner"/></Link>
